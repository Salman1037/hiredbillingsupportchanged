...
// Card component for challenges
function ChallengeItem({ title, desc, icon = <FaCheckCircle style={{ color: "#5CBDAA", marginRight: 10 }} /> }) {
  return (
    <li className="mb-4 d-flex align-items-start">
      {icon}
      <div>
...
// Card component for service section
function VirtualServiceCard({ icon, title, bullets }) {
  return (
...
// Card component for Why Choose section
function CardItem({ icon, title, desc }) {
  return (
    <div className="col-12 col-md-4 mb-4 d-flex align-items-stretch">
      <div className="card h-100 border-0 shadow-sm p-4 text-center" style={{ borderRadius: 16 }}>
        <div style={{ fontSize: 36, color: "#5CBDAA", marginBottom: 10 }}>{icon}</div>
        <h5 style={{ fontWeight: 700, fontSize: "1.15rem", marginBottom: 8 }}>{title}</h5>
        <p style={{ color: "#444", fontSize: "1rem", marginBottom: 0 }}>{desc}</p>
      </div>
    </div>
  );
}
...
const VirtualHealthcare = () => {
  ...
  return (
    <>
      ...
      >
        <div className="container text-start" style={{ maxWidth: 1200, margin: "0 auto", padding: "0 16px" }}>
          <h1 className="fw-bold mb-3" style={{ fontSize: "48px", lineHeight: "1.2" }}>Virtual Healthcare Solutions</h1>
          <div style={{ fontSize: "22px", fontWeight: 600, marginBottom: "15px" }}>On-Demand | Scalable | Cost-Effective</div>
          <p style={{ fontSize: "20px", lineHeight: "1.8", marginBottom: "30px", color: "#f0f0f0" }}>
            We help medical providers hire trained, HIPAA-compliant virtual healthcare professionals who work remotely to support daily operations, revenue cycle, and growth — without the overhead of in-house hiring.
          </p>
          <button
            className="btn"
            style={{ background: "#5CBDAA", color: "#fff", fontWeight: "bold", fontSize: "18px", padding: "14px 35px", borderRadius: "12px", border: "none", transition: "all 0.3s ease" }}
            onMouseOver={(e) => (e.target.style.background = "#4a9d93")}
            onMouseOut={(e) => (e.target.style.background = "#5CBDAA")}
            onClick={() => navigate("/contact")}
          >
      ...
      <section className="py-5">
        <div className="container">
          <h2 className="mb-4 fw-bold">Our Virtual Healthcare Services</h2>
          <div className="row">
            {VirtualServiceCards.map((card, i) => (
              <VirtualServiceCard key={i} icon={card.icon} title={card.title} bullets={card.bullets} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-5" style={{ background: "#f8f9fa" }}>
        <div className="container">
          <h2 className="mb-4 fw-bold">Common Challenges We Solve</h2>
          <ul className="list-unstyled">
            {VirtualChallenges.map((c, i) => (
              <ChallengeItem key={i} title={c.title} desc={c.desc} />
            ))}
          </ul>
        </div>
      </section>
      ...
      <section className="py-5">
        <div className="container">
          <h2 className="mb-4 fw-bold">Why Choose Our Virtual Healthcare Talent</h2>
          <div className="row">
            <CardItem icon={<FaRegClock />} title="Flexible Scheduling" desc="Access to healthcare professionals on-demand, scaling as your needs change." />
            <CardItem icon={<FaShieldAlt />} title="HIPAA Compliance" desc="All virtual staff trained in HIPAA standards to ensure patient confidentiality." />
            <CardItem icon={<FaRocket />} title="Cost-Effective" desc="Reduce overhead costs with skilled virtual staff without sacrificing quality." />
          </div>
        </div>
      </section>
      ...
      <section className="py-5" style={{ background: "#f8f9fa" }}>
        <div className="container">
          <h2 className="mb-4 fw-bold">Best For</h2>
          <ul className="list-unstyled">
            <li>Small & Medium Healthcare Practices</li>
            <li>Medical Billing Companies</li>
            <li>Telehealth Providers</li>
            <li>Medical Startups & Clinics Scaling Operations</li>
          </ul>
        </div>
      </section>
    </>
